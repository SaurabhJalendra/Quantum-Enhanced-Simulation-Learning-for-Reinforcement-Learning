"""Run remaining uniform_ensemble CartPole experiments: seeds 456, 789, 1024."""
import sys
import os
import logging
import gc
...
# Setup logging
LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'uniform_remaining.log')
logging.basicConfig(
    level=logging.INFO,
...
def compute_ensemble_loss(model, obs_seq, action_seq, reward_seq, kl_weight=KL_WEIGHT):
    ...
    if "all_states" in states:
        ...
        for i, ms in enumerate(states["all_states"]):
            ...
            rpred = model.models[i].reward_pred(combined).squeeze(-1)
...
def evaluate(model, buffer, num_batches=10):
    total_mse = 0.0
    total_rmse = 0.0
    model.eval()
    with torch.no_grad():
        for _ in range(num_batches):
            ...
            fs = states["all_states"][0] if "all_states" in states else states
            combined = torch.cat([fs["deter"], fs["stoch"]], dim=-1)
            rpred = model.models[0].reward_pred(combined).squeeze(-1)
            total_rmse += F.mse_loss(rpred, rewards).item()
    return {"obs_mse": total_mse / num_batches, "reward_mse": total_rmse / num_batches}


def eval_horizon(model, buffer, horizons=None):
    if horizons is None:
        horizons = [5, 10, 15, 20]
    results = {}
    model.eval()
...
def run_seed(seed):
    log.info(f"  [uniform_ensemble] seed={seed}")
    set_seed(seed)
    start = time.time()
    ...
    for ep in train_eps[:50]:
        eval_buffer.add(ep)
    train_metrics = evaluate(model, eval_buffer)
    ...
    for ep in test_eps:
        test_buffer.add(ep)
    test_metrics = evaluate(model, test_buffer)
    horizon_results = eval_horizon(model, test_buffer)
    ...
    result = {
        "approach": "uniform_ensemble", "seed": seed,
    ...
    # Aggressive cleanup
    del model, optimizer, buffer, eval_buffer, test_buffer
    gc.collect()
    torch.cuda.empty_cache()
    log.info(f"    GPU memory freed")
...
def main():
    ...
    log.info("=" * 60)
    log.info("Remaining Uniform Ensemble: seeds 456, 789, 1024")
    log.info("=" * 60)
