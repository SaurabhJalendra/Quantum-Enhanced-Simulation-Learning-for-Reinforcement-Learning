"""
Phase 1: Pendulum-v1 Experiment
================================

Runs ALL 5 approaches on Pendulum-v1 with 5 seeds each (25 total runs).
...
Environment: Pendulum-v1
    ...
    - action_dim = 1 (continuous torque in [-2, 2])
...
OBS_DIM = 3       # Pendulum: cos(theta), sin(theta), theta_dot
ACTION_DIM = 1    # Continuous torque in [-2, 2]
STOCH_DIM = 64
DETER_DIM = 512
HIDDEN_DIM = 512
...
# Data Collection
# ---------------------------------------------------------------------------
def collect_pendulum_episodes(num_episodes: int, seed: int) -> List[Dict]:
...
# ---------------------------------------------------------------------------
# Base World Model (used by baseline, tunneling, superposition)
# ---------------------------------------------------------------------------
class BaseWorldModel(nn.Module):
    ...
    def observe(
        self, obs: torch.Tensor, action: torch.Tensor, state: RSSMState
    ):
...
# ---------------------------------------------------------------------------
# Interference Ensemble Wrapper (approach 5)
# ---------------------------------------------------------------------------
...
def compute_ensemble_loss(
    ensemble_model: InterferenceWorldModel,
    obs_seq: torch.Tensor,
    action_seq: torch.Tensor,
    reward_seq: torch.Tensor,
    kl_weight: float = KL_WEIGHT,
...
# Model Factory
# ---------------------------------------------------------------------------
def create_model_for_approach(approach: str, seed: int) -> nn.Module:
...
def run_single(approach: str, seed: int, train_episodes: List[Dict]) -> Dict:
    ...
    This function handles the full pipeline: buffer creation, model
    creation, training, and evaluation (train, test, long-horizon).
    ...
    train_episodes : List[Dict]
        Pre-collected training episodes
    ...
    # Build buffer based on approach
    ...
    # Evaluate on test data (different seed offset for unseen data)
...
def main():
    print("=" * 70)
    print("PHASE 1: PENDULUM-v1 - ALL 5 APPROACHES x 5 SEEDS")
    print("=" * 70)
    print(f"Device: {DEVICE}")
    ...
    print(
        f"Architecture: obs={OBS_DIM}, act={ACTION_DIM}, "
        f"stoch={STOCH_DIM}, deter={DETER_DIM}, hidden={HIDDEN_DIM}"
    )
    print(
        f"Training: batch={BATCH_SIZE}, seq_len={SEQ_LEN}, "
        f"lr={LEARNING_RATE}, kl_weight={KL_WEIGHT}, grad_clip={GRAD_CLIP}"
    )
    print()
    ...
    for approach in APPROACHES:
        ...
        for seed in EXPERIMENT_SEEDS:
            ...
            print(
                ...
                f"(seed={seed}, {NUM_EPISODES} episodes)..."
            )
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("AGGREGATED RESULTS - PENDULUM-v1")
    print("=" * 70)
    ...
    for approach in APPROACHES:
        approach_results = [
            ...
            if r.get("approach") == approach and "error" not in r
        ]
        ...
        print(
            f"  Train Obs MSE: {np.mean(train_mses):.6f} "
            f"+/- {np.std(train_mses):.6f}"
        )
        print(
            f"  Test  Obs MSE: {np.mean(test_mses):.6f} "
            f"+/- {np.std(test_mses):.6f}"
        )
        print(
            f"  Test  Rew MSE: {np.mean(reward_mses):.6f} "
            f"+/- {np.std(reward_mses):.6f}"
        )
        print(f"  Avg Time:      {np.mean(times):.1f}s")
        print(f"  Parameters:    {approach_results[0]['num_params']:,}")
        if long_horizon_means:
            print("  Long-Horizon MSE:")
            for h_key, stats in sorted(
                long_horizon_means.items(), key=lambda x: int(x[0])
            ):
                print(
                    f"    h={h_key}: {stats['mean']:.6f} "
                    f"+/- {stats['std']:.6f}"
                )
    ...
    print(f"Individual results: {results_dir}/<approach>_seed_<seed>.json")
    print("DONE")
