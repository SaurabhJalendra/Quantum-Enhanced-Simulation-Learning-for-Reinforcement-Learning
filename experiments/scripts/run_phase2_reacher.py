"""
Phase 2: Reacher-easy DMControl Experiment
==========================================

Runs ALL 5 approaches on the Reacher-easy environment with 5 seeds each.
...
OBS_DIM = 6  # Reacher-easy: position(2) + to_target(2) + velocity(2)
ACTION_DIM = 2  # Continuous [-1, 1]
STOCH_DIM = 64
DETER_DIM = 512
HIDDEN_DIM = 512
...
def flatten_obs(obs: dict) -> np.ndarray:
    """Flatten dm_control observation dict into a single vector."""
    parts = []
...
# Data Collection
# ---------------------------------------------------------------------------
def collect_episodes(domain: str, task: str, num_episodes: int, seed: int) -> List[Dict]:
    ...
    for i in range(num_episodes):
        ...
        while not timestep.last():
            ...
            obs_list.append(obs)
            action = np.random.uniform(-1, 1, size=env.action_spec().shape).astype(np.float32)
...
def compute_ensemble_loss(
    ...
    total = 0.5 * combined_recon_loss + 0.5 * individual_loss + kl_weight * kl_loss - 0.01 * diversity
    return {
        "total": total,
...
def evaluate_model(
    ...
    with torch.no_grad():
        for _ in range(num_eval_batches):
            ...
            # Reward MSE
            if approach == "interference_ensemble":
                # Use first ensemble model for reward prediction
...
def run_single(approach: str, seed: int, train_episodes: List[Dict]) -> Dict:
    ...
    test_episodes = collect_episodes("reacher", "easy", 50, seed + 10000)
...
def main():
    print("=" * 70)
    print("PHASE 2: REACHER-EASY - ALL 5 APPROACHES x 5 SEEDS")
    print("=" * 70)
    print(f"Device: {DEVICE}")
    ...
    results_dir = PROJECT_ROOT / "experiments" / "results" / "phase2" / "reacher"
    ...
    for approach in APPROACHES:
        ...
        for seed in EXPERIMENT_SEEDS:
            ...
            train_episodes = collect_episodes("reacher", "easy", NUM_EPISODES, seed)
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("AGGREGATED RESULTS - REACHER-EASY")
    print("=" * 70)
    ...
    complete_metrics = {
        "experiment": "phase2_reacher_easy",
        "environment": {"domain": "reacher", "task": "easy", "obs_dim": OBS_DIM, "action_dim": ACTION_DIM},
        "config": {
            "stoch_dim": STOCH_DIM,
            "deter_dim": DETER_DIM,
            "hidden_dim": HIDDEN_DIM,
