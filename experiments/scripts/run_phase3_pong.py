"""
Phase 3: Atari Pong Experiment
===============================

Runs ALL 5 approaches on Atari Pong with 5 seeds each.

Key differences from Phase 2 (DMControl):
    - Pixel observations (84x84 grayscale) require CNN encoder/decoder
    - Discrete actions (6) with one-hot encoding
...
try:
    ...
    # Verify Pong ROM is available
    _test_env = gym.make("ALE/Pong-v5", render_mode=None)
    _test_env.close()
    del _test_env
except Exception as e:
    print(f"ERROR: Atari/Pong ROM not available: {e}")
...
ACTION_DIM = 6  # Pong discrete actions
STOCH_DIM = 64
DETER_DIM = 512
HIDDEN_DIM = 512
...
def collect_episodes(env_name: str, num_episodes: int, seed: int) -> List[Dict]:
    """Collect episodes from Atari Pong using random policy."""
...
def main():
    print("=" * 70)
    print("PHASE 3: ATARI PONG - ALL 5 APPROACHES x 5 SEEDS")
    print("=" * 70)
    print(f"Device: {DEVICE}")
    ...
    results_dir = PROJECT_ROOT / "experiments" / "results" / "phase3" / "pong"
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("AGGREGATED RESULTS - ATARI PONG")
    print("=" * 70)
